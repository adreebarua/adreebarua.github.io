...
let flappyFont, gameFont, explosion, jump, score, hurt, music, cheers
...
function preload() {
    ...
    cheers = loadSound('sounds/cheers.wav')
...
function done() {
    ...
    text("Press Enter to Play Agian", width / (2 * 1.74), 200)
}
...
function play() {
    ...
    if (spawnTimer > 2) {
        ...
        pipe.y = constrain(lastY + random(-20, 20), 100, 200)
        lastY = pipe.y
        pipes.push(pipe)
        spawnTimer = 0
    }
